...
#SBATCH --job-name=dc_rnn_dmlab-target_data-startat0-pred_v-linear_increasing-causal-anneal_gt
#SBATCH --output=./logs/dc_rnn_dmlab/dc_rnn_dmlab-target_data-startat0-pred_v-linear_increasing-causal-anneal_gt-%j.out
#SBATCH --error=./logs/dc_rnn_dmlab/dc_rnn_dmlab-target_data-startat0-pred_v-linear_increasing-causal-anneal_gt-%j.err
...
# python -m main +name=dc_rnn_dmlab-target_data-startat0-pred_v-linear_increasing-causal-anneal_gt algorithm=rnn_dc_video experiment=exp_video dataset=video_dmlab algorithm.original_algo.model.num_gru_layers=0 dataset.context_length=4 dataset.n_frames=36 dataset.frame_skip=1 "algorithm.original_algo.ckpt_path=/weka/s224075134/temporal_diffusion/diffusion-forcing/outputs/2025-03-10/07-25-58/checkpoints/epoch\=0-step\=200000.ckpt"
...
python -m main +name=dc_rnn_dmlab-target_data-startat0-pred_v-linear_increasing-causal-anneal_gt algorithm=rnn_dc_video experiment=exp_video \
        dataset=video_dmlab dataset.context_length=4 dataset.n_frames=24 dataset.frame_skip=1 \
        algorithm.target_type=data algorithm.random_start=start algorithm.diffusion.objective=pred_v algorithm.finetune_org_model=False \
