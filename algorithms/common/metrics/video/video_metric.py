...
from .element_wise_ssim import ElementWiseSSIM
...
from .psnr import FrameWise_PSNR
...
class VideoMetric(nn.Module):
    ...
    def __init__(
        ...
        registry: SharedVideoMetricModelRegistry = None,
            ...
            match metric_type:
                case VideoMetricType.LPIPS:
                    module = LearnedPerceptualImagePatchSimilarity(
                        registry=registry, normalize=True, frame_wise=frame_wise, max_frames=max_frames
                    )
                case VideoMetricType.FID:
                    module = FrechetInceptionDistance(registry=registry, normalize=True, frame_wise=frame_wise, max_frames=max_frames)
                ...
                case VideoMetricType.MSE:
                    module = MeanSquaredError(num_outputs=max_frames if frame_wise else 1)
                case VideoMetricType.SSIM:
                    if frame_wise:
                        module = ElementWiseSSIM(data_range=1.0, reduction='elementwise_mean', n_frames=max_frames)
                    else:
                        module = StructuralSimilarityIndexMeasure(data_range=1.0)
                    module.frame_wise = frame_wise
                case VideoMetricType.PSNR:
                    if frame_wise:
                        module = FrameWise_PSNR(dim=(1, 2, 3, 4), data_range=1.0, reduction='none', max_frames=max_frames)
                    else:
    ...
    def _update(
        ...
        if self.frame_wise:
            preds, target = map(
                lambda x: rearrange(x[:, ~context_mask], "b t c h w -> t b c h w"),
                (preds, target),
            )
        else:
            preds, target = map(
    ...
    def log(self, prefix: str):
        ...
        for metric_type, module in self._filtered_items(self.VBENCH_METRICS, not_in=True):
            ...
            if hasattr(module, "frame_wise") and module.frame_wise:
                for i in range(output.shape[0]):
                    dict_metrics[f"{prefix}/{metric_type}/frame_{i}"] = output[i]
            else:
