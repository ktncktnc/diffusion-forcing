...
def _mean_squared_error_update(preds: Tensor, target: Tensor, num_outputs: int) -> tuple[Tensor, int]:
    ...
    if num_outputs == 1:
        ...
        diff = preds - target
        sum_squared_error = torch.sum(diff * diff, dim=[0,2,3,4])
...
class MeanSquaredError(Metric):
    ...
    def update(self, preds: Tensor, target: Tensor) -> None:
        """Update state with predictions and targets. Receive preds and target with timestep first."""
        if self.num_outputs > 1:
            preds = rearrange(preds, "t b ... -> b t ...")
            target = rearrange(target, "t b ... -> b t ...")
        else:
