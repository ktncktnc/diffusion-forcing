...
class Conv2dGRUCell(ConvRNNCell):
    ...
    def forward(self, input, hx):
        x_t = self.x2h(input) # (batch_size, in_channels * channel_multiplier, height_size, width_size)
        h_t = self.h2h(hx) # (batch_size, in_channels * channel_multiplier, height_size, width_size)

        x_reset, x_upd, x_new = x_t.chunk(3, 1) # (batch_size, in_channels, height_size, width_size)
        h_reset, h_upd, h_new = h_t.chunk(3, 1) # (batch_size, in_channels, height_size, width_size)
